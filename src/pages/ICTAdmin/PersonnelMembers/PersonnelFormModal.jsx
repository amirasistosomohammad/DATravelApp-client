...
const PersonnelFormModal = ({
  editingPersonnel,
  existingPersonnel: propExistingPersonnel = [],
  onSubmit,
  onClose,
  actionLock,
}) => {
  ...
  const [existingPersonnel, setExistingPersonnel] = useState(propExistingPersonnel);
  const [personnelLoading, setPersonnelLoading] = useState(propExistingPersonnel.length === 0);
    ...
    username: (value) => {
      ...
      const duplicate = existingPersonnel.find(
        (p) =>
          ...
          (!isEdit || p.id !== editingPersonnel?.id)
      );
      if (duplicate) {
  ...
  const fetchExistingPersonnel = useCallback(async () => {
    // If existingPersonnel was passed as prop, use it instead of fetching
    if (propExistingPersonnel.length > 0) {
      setExistingPersonnel(propExistingPersonnel);
    ...
    try {
      const response = await fetch(
        `${
          import.meta.env.VITE_LARAVEL_API || "http://localhost:8000/api"
        }/ict-admin/personnel?per_page=1000`,
        {
          headers: {
  ...
  useEffect(() => {
    if (editingPersonnel) {
      const existingAvatar = resolveAvatarUrl(editingPersonnel);
      console.log("Personnel object:", editingPersonnel);
      console.log("Avatar path:", editingPersonnel.avatar_path);
      console.log("Resolved avatar URL:", existingAvatar);
      const phoneValue = editingPersonnel.phone || "";
      ...
      const firstName =
        editingPersonnel.first_name ||
        (editingPersonnel.name ? editingPersonnel.name.split(" ")[0] : "") ||
        "";
      const lastName =
        editingPersonnel.last_name ||
        (editingPersonnel.name
          ? editingPersonnel.name.split(" ").slice(1).join(" ")
          : "") ||
        "";

      const personnelFormState = {
        username: editingPersonnel.username || "",
        first_name: firstName,
        middle_name: editingPersonnel.middle_name || "",
        last_name: lastName,
        phone: formattedPhone,
        department: editingPersonnel.department || "",
        position: editingPersonnel.position || "",
        password: "",
        password_confirmation: "",
        is_active: editingPersonnel.is_active !== false, // Default to true if not explicitly false
        reason_for_deactivation: editingPersonnel.reason_for_deactivation || "",
      };
  ...
  }, [editingPersonnel, resolveAvatarUrl, updateAvatarPreview]);
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmation = await Swal.fire({
      icon: "question",
      title: isEdit ? "Update Personnel?" : "Create Personnel?",
      text: isEdit
      ...
      confirmButtonText: isEdit ? "Update Personnel" : "Create Personnel",
    ...
    try {
      Swal.fire({
        title: isEdit ? "Updating Personnel" : "Creating Personnel",
        text: "Please wait while we save the personnel information...",
      ...
      const url = isEdit
        ? `${apiBase}/ict-admin/personnel/${editingPersonnel.id}`
        : `${apiBase}/ict-admin/personnel`;
      ...
      if (response.ok && data.success) {
        await Swal.fire({
          icon: "success",
          title: isEdit ? "Personnel Updated" : "Personnel Created",
          text: isEdit
        ...
        if (onSubmit) {
          onSubmit(data.data || data.personnel || data);
        }

        const refreshedAvatar = resolveAvatarUrl(data.data || data.personnel || data);
        const normalizedPersonnel = {
          username: (data.data || data.personnel || data).username || "",
                ...
                <div className="row g-3">
                    ...
                    <div className="col-12">
                      ...
                      className={`form-control ${
                                  ...
                                  onChange={handleChange}
                                  disabled={loading}
                                  placeholder="Enter the reason for deactivation (this will be shown to the personnel when they try to login)"
                                  rows={3}
                                  style={{
                                    backgroundColor: "var(--input-bg)",
                                ...
                                <small className="text-muted mt-1 d-block">
                                  ...
                                  This reason will be displayed to the personnel when they attempt to login.
                                </small>
                              </motion.div>
                            )}
                          </AnimatePresence>
